...
int CalculateGCD (int[] numbers) {
   int gcd = numbers[0];
   for (int i = 1; i < numbers.Length; i++) gcd = numbers[i] == 0 ? gcd : CalculateGCDforTwoNumbers (gcd, numbers[i]);
   return gcd;
}

int CalculateGCDforTwoNumbers (int a, int b) => (b == 0) ? a : CalculateGCDforTwoNumbers (b, a % b);

int CalculateLCM (int[] numbers) {
   int lcm = numbers[0];
   for (int i = 1; i < numbers.Length; i++) lcm = (lcm * numbers[i]) / CalculateGCDforTwoNumbers (lcm, numbers[i]);
   return lcm;
}