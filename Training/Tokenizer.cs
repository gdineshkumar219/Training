   ...
   public Token Next (List<Token> tokens) {
      while (mN < mText.Length) {
         ...
         switch (ch) {
            ...
            case '+' or '-':
               if (tokens.Count == 0 || tokens[^1] is TOperator { Priority: _ } || (tokens[^1] is TPunctuation { Punct: '(' }))
                  return new TOpUnary (mEval, ch);
               return new TOpArithmetic (mEval, ch);
            case '*' or '/': return new TOpArithmetic (mEval, ch);
            case '=': return new TOpArithmetic (mEval, ch);
            case '^': return new TOpArithmetic (mEval, ch);
